...
### 1. Open API
  - https://developers.kakao.com/docs/latest/ko/getting-started/app 에서 API 키를 발급 받아야 합니다. 
  - 직접 빌드 시
    - `src/main/resources/application.yml` 의 open.kako.REST_API_KEY의 {API_KEY} 를 발급받은 API 키로 변경합니다.
  - 빌드 파일 실행 시
    - 이곳 에서 jar 파일을 다운받고 다운받은 파일이 있는 위치에서 아래 명령어를 실행합니다.
...
- 필요 시 타 벤더사로 교체할 수 있도록 API 호출 로직 추상화
### 검색어 랭킹
